...
CPU-риск Astro SSR проверен 30 июля 2026 года на preview. Для активной версии после
36 вызовов CPU Time P50/P90/P99 составил 0,90/3,06/4,58 ms; resource errors,
exceeded CPU и exceeded memory отсутствовали. Статические страницы обслуживаются
без запуска Worker.

После production cutover метрики нужно проверить повторно на реальном трафике:

1. нет `exceededResources`;
2. CPU Time P50/P90/P99 остаются ниже лимита;
...
4. Wall Time оценивается отдельно от CPU: внешний Render Free может давать холодный
   старт и временный HTTP 503.
